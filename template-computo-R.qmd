...
This document, accompanied with the [hopefully finely tuned git repos](https://github.com/computorg/template-computo-R/), provides a template based on the [quarto system](https://quarto.org/) for contributions to **Computo** @computo. We show how `R` [@R-base] code can be included and how the repository can be set-up for triggering github-action with `renv`.
...
This section covers basic formatting guidelines with [Quarto](https://quarto.org/), a versatile formatting system for authoring HTML based on markdown, integrating LaTeX and various code block interpreted either via Jupyter or Knitr (and thus deal with Python, R and many other langages). It relies on the [Pandoc Markdown](https://rmarkdown.rstudio.com/authoring_pandoc_markdown.html) markup language.
...
- Also look at the previously published paper in Computo for even mroe advanced examples.
...
To make your work reproducible, you need to fix the packages and environment used to run your analysis. For the `R` system, the `renv` package is a reliable method, supported by the `quarto` community. You basically need a couples of command to setup your environment on your local machine:

```{r, eval = FALSE}
renv::init()
```

will intialize your repository. Then you just need to install the dependencies required to run your contribution, for instance, 
...
Once you are down, you can fix everything via the commmand
...
- on the [`renv` packge webpage](https://rstudio.github.io/renv/articles/renv.html)
- on the [quarto page dedicated to environments](https://quarto.org/docs/projects/virtual-environments.html#using-renv)

## Continuous integration

The repository associated with this template is preconfigure to trigger an action on push that performs the following:
...
Once this is succesful, you are ready to submit your manuscript to the [Computo submission platform](https://computo.scholasticahq.com/).
